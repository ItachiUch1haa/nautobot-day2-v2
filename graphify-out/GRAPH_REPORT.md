...
## Summary
- 489 nodes · 938 edges · 35 communities (27 shown, 8 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 69 edges (avg confidence: 0.86)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f9261ad8`
...
3. `sync_device()` - 15 edges
4. `process_csv()` - 15 edges
...
- `fetch_device_credential()` --calls--> `fetch_openbao_secret()`  [INFERRED]
  nautobot_day2/broker/core.py → nautobot_day2/openbao_client.py
- `run_diagnostic_commands()` --calls--> `_find_site_firewall_ip()`  [INFERRED]
  nautobot_day2/broker/core.py → nautobot_day2/onboarding/sync_network_data.py
...
Cohesion: 0.08
Nodes (36): custom_route, device_info(), diagnose(), diagnose_batch(), health(), metrics(), route, broker/api_server.py Agent Broker — REST API wrapper. Thin HTTP layer over… (+28 more)
...
Cohesion: 0.12
Nodes (31): api_post(), association_exists(), create_external_integrations(), create_namespace(), create_secrets_groups(), create_tenant_record(), derive_objects(), _existing_env_vars() (+23 more)
...
Cohesion: 0.19
Nodes (20): api_post(), create_custom_field(), create_device_roles(), create_location_types(), create_manufacturers(), create_platforms(), create_tags(), create_tenant_groups() (+12 more)
...
Cohesion: 0.21
Nodes (16): api_get_all(), get_devices(), get_devices_for_site(), get_yaml_block(), init_cache(), load_last_failed(), load_vendor_commands(), main() (+8 more)
...
Cohesion: 0.14
Nodes (15): derive_platform(), Derive default platform slug from vendor+device_type., api_vendor_platforms(), Return valid platforms for vendor+role combo., get_default_platform(), get_platforms_for_combo(), get_roles_for_vendor(), get_secrets_group_prefix() (+7 more)
...
Cohesion: 0.17
Nodes (8): extract_interfaces(), _find_site_firewall_ip(), For Fortinet AP rows using the fortinet_ap_ssh fallback: a real FortiAP has no…, Resolve (platform_slug, role_name, sg_name) → (section, yaml_key). Priority: 1.…, Parse vendor-specific interface output into structured list. Returns list of…, resolve_vendor(), sync_device(), SyncResult
...
Cohesion: 0.29
Nodes (7): api_post(), _assign_ip_to_interface(), _find_interface(), Create IP address and link it to interface if not already existing., Find interface ID by name with fallback normalization: - Exact match:…, Create cables in Nautobot from LLDP neighbor data. 3-level matching: 1.…, write_cables()
...
Cohesion: 0.50
Nodes (4): api_patch(), get_active_status_id(), write_facts(), write_interfaces()
...
Cohesion: 0.50
Nodes (4): api_save_credentials(), _find_external_integration_label(), Given a base env var name (no tenant suffix), find the 'external_integration'…, Write submitted credential values into a tenant's .env file. Only variable…

### Community 26 - "Community 26"
Cohesion: 0.50
Nodes (4): api_vendors(), Return enabled vendors from vendor_matrix., get_enabled_vendors(), Returns list of vendor slugs that have at least one enabled device type.
...
- **Why does `NautobotClient` connect `Community 3` to `Community 1`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `api_validate_credentials()` connect `Community 14` to `Community 6`, `Community 7`, `Community 9`, `Community 13`, `Community 16`?**
  _High betweenness centrality (0.050) - this node is a cross-community bridge._
- **Why does `get_device_context()` connect `Community 1` to `Community 3`?**
